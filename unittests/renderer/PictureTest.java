...
public class PictureTest {
    ...
    private List<Sphere>  initializeBalls() {
        ...
        for(double i = 0,j = 6; i <500 ;i+=1,j+=0.04){
            double x = Math.cos(i);
            double y = Math.sin(i);
            balls.add((Sphere) new Sphere(new Point(i*y, i*x, (i*2 - 150)), j).setMaterial(material1).setEmission(new Color(255,0,0)));
        }
        return balls;
    }
    @Test
    public void PictureTest() {
        ...
        camera.setImageWriter(new ImageWriter("final picture", 3000, 3000))
